...
export function ApiProvider({ children }) {
  ...
  const editUser = async (userId, updatedData) => {
    ...
    try {
      ...
      toast(response.message || "User updated successfully");
      navigate(-1);
    } catch (err) {
      handleApiError(err);
    } finally {
  ...
  const deleteBranch = async (branchId) => {
    ...
    try {
      ...
      fetchBranches();
      toast(response.message || "Branch has been deleted successfully");
    } catch (err) {
      navigate("/admin/branches");
      handleApiError(err);
    } finally {
  ...
  const assignTask = async (task) => {
    ...
    try {
      ...
      toast(response.message || "Task assigned successfully");
          fetchUserOrAdmin();
      navigate('/admin/users');
    } catch (err) {
      handleApiError(err);
  ...
  const submitTask = async (taskId, taskStatusId) => {
    ...
    try {
      ...
      toast(response.message || "Task submitted successfully");
      fetchTasksForUser();
    } catch (err) {
      handleApiError(err);
    } finally {
  ...
  if (loading) return <Loader size="lg" text="Please wait..." />
